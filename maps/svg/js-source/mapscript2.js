...
(function () {
    ...
    ixMap.Event.prototype.doDefaultZoom = function (evt) {
        ...
        if (this.fFrozenDynamicContent ||
            ...
            try {
                var __ix = map.HTMLWindow.ixmaps;
                if (__ix && typeof __ix.htmlgui_onZoomAndPan === "function") {
                    __ix.htmlgui_onZoomAndPan(map.Scale.nZoomScale);
                }
                if (__ix && typeof __ix.__dispatchMapViewEvents === "function") {
                    __ix.__dispatchMapViewEvents({
                        nZoom: map.Scale.nZoomScale,
                        zoomChanged: map.Scale.oldZoomScale != map.Scale.nZoomScale,
                        panChanged: Math.abs(map.Scale.oldMapCenter.x - map.Scale.mapCenter.x) > 10 ||
                            Math.abs(map.Scale.oldMapCenter.y - map.Scale.mapCenter.y) > 10,
                        frozenDynamic: this.fFrozenDynamicContent,
                        szMap: __ix.szName || null
                    });
                }
            } catch (e) {}
